...
def policy_evaluation(P, nS, nA, policy, gamma=0.9, tol=1e-3):
	...
	while True:
		...
		for state in range(nS):
			action = policy[state]
			reward = 0
			for probability, nextstate, innermediate_reward, terminal in P[state][action]:
				reward += probability * (innermediate_reward + gamma * prev_value_function[nextstate])
...
def policy_improvement(P, nS, nA, value_from_policy, policy, gamma=0.9):
	...
	for state in range(nS):
		max_value, max_arg = float('-inf'), -1
		for action in range(nA):
			cur_value = 0
			for probability, nextstate, reward, terminal in P[state][action]:
				cur_value += probability * (reward + gamma * value_from_policy[nextstate])
			if cur_value > max_value:
				max_value = cur_value
				max_arg = action
		new_policy[state] = max_arg
	############################
	return new_policy
	...
	while True:
		...
		for state in range(nS):
			for action in range(nA):
				...
				for probability, nextstate, innermediate_reward, terminal in P[state][action]:
					reward += probability * (innermediate_reward + gamma * prev_value_function[nextstate])
				value_function[state] = max(value_function[state], reward)
		if np.max(np.abs(value_function-prev_value_function)) < tol:
